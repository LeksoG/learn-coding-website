...
async function askAIForErrorFix(errorId) {
    ...
    // Add user message
    addAIMessage(`🐛 Fix this error: ${errorMessage}`, 'user');
    ...
    try {
        if (isAgentMode && code.trim()) {
            // Agent Mode: Fix the error and apply code
            const response = await callMistralAI(
                `Please fix this error in the code: "${errorMessage}"\n\nCurrent code:\n${code}\n\nProvide ONLY the complete corrected code without any explanation.`,
                code,
                true
            );
            ...
            await applyCodeWithAnimation(newCode, 'fixed the error');
        } else {
            // Normal Mode: Explain the error and suggest fix
            const response = await callMistralAI(
                `Please help fix this error: "${errorMessage}"\n\nCode:\n${code}\n\nExplain what's wrong and provide the corrected code.`,
                code,
                false
            );
            removeTypingIndicator(typingId);
        ...
        removeTypingIndicator(typingId);
        addAIMessage('Sorry, I encountered an error analyzing your code. Please try again.', 'assistant');
    }
}
...
async function askAIForErrorFromPopup() {
    closeErrorPopup();
    ...
    // Add user message
    addAIMessage(`🐛 Fix this error: ${lastErrorMessage}`, 'user');
    ...
    try {
        if (isAgentMode && code.trim()) {
            // Agent Mode: Fix the error and apply code
            const response = await callMistralAI(
                `Please fix this error in the code: "${lastErrorMessage}"\n\nCurrent code:\n${code}\n\nProvide ONLY the complete corrected code without any explanation.`,
                code,
                true
            );
            removeTypingIndicator(typingId);
            ...
            // Normal Mode: Explain the error and suggest fix
            const response = await callMistralAI(
                `Please help fix this error: "${lastErrorMessage}"\n\nCode:\n${code}\n\nExplain what's wrong and provide the corrected code.`,
                code,
                false
            );
            removeTypingIndicator(typingId);
...
</script>        } else if (this.animationPhase === 'reveal') {
            this.animateReveal();
        }
...
function showSectionSidebar(sectionId) {
    ...
    // Use CSS classes only - no JS layout changes
    const sections = document.querySelectorAll('.section');
    const targetSection = document.getElementById(sectionId);
    
    // CRITICAL: All changes in ONE batch
    requestAnimationFrame(() => {
        sections.forEach(s => {
            if (s === targetSection) {
    ...
    // Lazy load heavy content AFTER transition
    if (sectionId === 'home') {
        setTimeout(() => {
